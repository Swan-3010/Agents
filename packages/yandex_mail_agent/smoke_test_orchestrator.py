...
class SmokeTestOrchestrator:
    ...
    def run_smoke_test(
        ...
        try:
            # Шаг 1: Получение чеков из IMAP с parser-based фильтрацией
        ...
        try:
            ...
            for msg in messages:
                ...
                if not decision.should_process:
                    logger.info(
                        "Письмо пропущено dispatcher/parser gate: message_id=%s reason=%s",
                        decision.message_id,
                        decision.reason,
                    )
                    continue
                ...
                if receipt_url:
                    receipts.append({
                        "url": receipt_url,
                        "subject": self._read_message_field(msg, "subject"),
                        "message_id": decision.message_id,
                    })

            logger.info(f"Получено {len(receipts)} чеков после parser/dispatcher фильтрации")
            return receipts
        except Exception as e:
                    ...
                    try:
                        ...
                        if result["success"]:
                            details.append({
                                ...
                                "URL": receipt["url"],
                                "Message-ID": receipt.get("message_id", ""),
    ...
    @staticmethod
    def _read_message_field(msg: Any, field_name: str, default: str = "") -> str:
        """Безопасное чтение поля сообщения для dict/object совместимости."""
        if isinstance(msg, dict):
            return str(msg.get(field_name, default) or default)
        return str(getattr(msg, field_name, default) or default)
